...
/*   Updated: 2021/11/24 22:57:43 by lahammam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_flags_left_min_decimal(char **str, char *temp, char *flags, char c)
{
	...
	if (ft_char_in_strig(flags,'-') != -1)
	{
		while (temp[++i])
			*str = ft_add_char(*str, temp[i]);
		if (ft_char_in_strig(flags,'+') != -1 || ft_char_in_strig(flags,' ') != -1)
...
void	ft_flags_left_min_hex(char **str, char *temp, char *flags, char c)
{
	...
	if (ft_char_in_strig(flags,'-') != -1)
	{
		while (temp[++i])
			*str = ft_add_char(*str, temp[i]);
		ft_add_n_char(str, c, ft_atoi(flags + ft_min_width(flags, 1)) - ft_strlen(temp));
	}
	else
	{
		ft_add_n_char(str, c, ft_atoi(flags + ft_min_width(flags, 1)) - ft_strlen(temp));
